...
//  TwitterApp
//
//  Created by Роман Елфимов on 26.10.2021.
//
...
class TweetHeader: UICollectionReusableView {
    ...
    // стрелка вниз напротив от кортинки профиля по правому краю
    ...
    private lazy var commentButton: UIButton = {
        let button = createButton(withImagName: "comment")
        button.addTarget(self, action: #selector(handleCommentTapped), for: .touchUpInside)
        return button
    }()
    
    private lazy var retweetButton: UIButton = {
        let button = createButton(withImagName: "retweet")
        button.addTarget(self, action: #selector(handleRetweetTapped), for: .touchUpInside)
        return button
    }()
    
    private lazy var likeButton: UIButton = {
        let button = createButton(withImagName: "like")
        button.addTarget(self, action: #selector(handleLikeTapped), for: .touchUpInside)
        return button
    }()
    
    private lazy var shareButton: UIButton = {
        let button = createButton(withImagName: "share")
